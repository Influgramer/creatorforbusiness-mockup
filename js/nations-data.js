...
class NationsDataProvider {
    static getLocationFromCountryCode(countryCode){
		if(countryCode==null){
			return null;
		}
        for (var i=0; i<NATIONS_DATA.length; i++) {
            const nation=NATIONS_DATA[i];
            if(nation.CountryCode==countryCode){
                return {
                    lat:parseFloat(nation.CapitalLatitude),
                    lng:parseFloat(nation.CapitalLongitude)
                };
            }
        }
		return null;
    }
	static getBrowserCountryCode(){
